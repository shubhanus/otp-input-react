...
import Input from "./Input"; // keyCode constants

const BACKSPACE = 8;
const LEFT_ARROW = 37;
const RIGHT_ARROW = 39;
const DELETE = 46; // Doesn't really check if it's a style Object
...
const OtpInput = ({
  ...
  const getOtpValue = () => value ? value.toString().split("") : []; // Helper to return OTP from input
  ...
  const handleOnPaste = e => {
    e.preventDefault();
    const otp = getOtpValue(); // Get pastedData in an array of max size (num of inputs - current position)

    const pastedData = e.clipboardData.getData("text/plain").slice(0, numInputs - activeInput).split(""); // Paste data from focused input onwards
  ...
  const renderInputs = () => {
    const otp = getOtpValue();
    const inputs = []; // eslint-disable-next-line no-plusplus
